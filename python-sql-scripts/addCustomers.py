...
connection = pymysql.connect(
    host=mysql_endpoint, user=username, passwd=password, db=database_name
)
...
create_disputelogs_table_sql = """
CREATE TABLE DisputeLogs (
...
"""
cursor.execute(create_disputelogs_table_sql)
...
for i in range(num_customers_to_add):
    ...
    if customer_id not in ids_in_db and customer_id not in ids_just_added:
        ...
        customer_balance = (
            random.randint(100, 10000) * 100
        )  # multiply by 100 to have a penny value of 0
        customer_password = "".join(
            random.choices(
                string.ascii_lowercase + string.ascii_uppercase + string.digits, k=9
            )
        )
