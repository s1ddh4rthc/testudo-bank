...
public class MvcController {
    ...
    public MvcController(@Autowired JdbcTemplate jdbcTemplate, @Autowired CryptoPriceClient cryptoPriceClient, @Autowired StockPriceClient stockPriceClient) {
    ...
    private void updateAccountInfo(User user) {
        List<Map<String, Object>> overdraftLogs = TestudoBankRepository.getOverdraftLogs(jdbcTemplate,
                user.getUsername());
        ...
        user.setNumDepositsForInterest(
                TestudoBankRepository.getCustomerNumberOfDepositsForInterest(jdbcTemplate, user.getUsername()));
    }
    ...
    public String submitDeposit(@ModelAttribute("user") User user) {
        ...
        int userDepositAmtInPennies = convertDollarsToPennies(userDepositAmt); // dollar amounts stored as pennies to
                                                                               // avoid floating point errors
        String currentTime = SQL_DATETIME_FORMATTER.format(new java.util.Date()); // use same timestamp for all logs
        ...
        if (userOverdraftBalanceInPennies > 0) { // deposit will pay off overdraft first
            ...
            TestudoBankRepository.insertRowToOverdraftLogsTable(jdbcTemplate, userID, currentTime,
                    userDepositAmtInPennies, userOverdraftBalanceInPennies, newOverdraftBalanceInPennies);
            ...
            if (userDepositAmtInPennies > userOverdraftBalanceInPennies) {
                ...
                TestudoBankRepository.increaseCustomerCashBalance(jdbcTemplate, userID,
                        mainBalanceIncreaseAmtInPennies);
            }
    ...
    public String submitWithdraw(@ModelAttribute("user") User user) {
        ...
        if (userWithdrawAmtInPennies > userBalanceInPennies) { // if withdraw amount exceeds main balance, withdraw into
            ...
            int newOverdraftBalanceInPennies = userOverdraftBalanceInPennies
                    + newOverdraftIncreaseAmtAfterInterestInPennies;
    ...
    public String submitDispute(@ModelAttribute("user") User user) {
        ...
        if (((String) logToReverse.get("Action")).toLowerCase().equals("deposit")) {
            ...
            if (userOverdraftBalanceInPennies
                    + (reversalAmountInPennies - userBalanceInPennies) > MAX_OVERDRAFT_IN_PENNIES) {
                return "welcome";
            }
            ...
            if (reversalAmountInPennies > userBalanceInPennies) {
                ...
                if (overdraftLogs.size() != 0) {
                    ...
                    TestudoBankRepository.deleteRowFromOverdraftLogsTable(jdbcTemplate, userID,
                            datetimeOfReversedDeposit);
                    TestudoBankRepository.setCustomerOverdraftBalance(jdbcTemplate, userID,
    ...
    public String buyCrypto(@ModelAttribute("user") User user) {
        ...
        if (withdrawResponse.equals("account_info")) {
            ...
            TestudoBankRepository.insertRowToCryptoLogsTable(jdbcTemplate, userID, cryptoToBuy,
                    CRYPTO_HISTORY_BUY_ACTION, currentTime, cryptoAmountToBuy);
    ...
    public String sellCrypto(@ModelAttribute("user") User user) {
        ...
        Optional<Double> cryptoBalance = TestudoBankRepository.getCustomerCryptoBalance(jdbcTemplate, userID,
                cryptoToBuy);
        ...
        if (depositResponse.equals("account_info")) {
            ...
            TestudoBankRepository.insertRowToCryptoLogsTable(jdbcTemplate, userID, cryptoToBuy,
                    CRYPTO_HISTORY_SELL_ACTION, currentTime, cryptoAmountToSell);
    ...
    // applies interst to users based on certain conditions when a user deposits
    ...
    public String applyInterest(@ModelAttribute("user") User user) {
        ...
        if (userBalance >= 0 && overdraftBalance <= 0 && depositValue >= 20) {
            ...
            if (numDeposits % 5 == 0) {
                ...
                TestudoBankRepository.insertRowToTransactionHistoryTable(jdbcTemplate, user.getUsername(), date,
                        TRANSACTION_HISTORY_DEPOSIT_ACTION, interestAmount);
                ret = "account_info";
            }
    ...
    /**
     * Method which is used to invest in the specified stock the user inputs
     * 
     * @param user the user
     * @return the string of the page we should render based on conditions
    */
    @PostMapping("/invest")
    public String buyStock(@ModelAttribute("user") User user) {
        ...
        String ticker = user.getWhichStockToInvest();
        double shares = user.getAmountToBuyStock();
        double price = stockPriceClient.getCurrentStockValue(ticker);
        double costOfStockPurchaseInDollars = shares * price;
        ...
        // should not happen but on the off chance it does
        ...
        String withdrawResponse = submitWithdraw(user);
        if (withdrawResponse.equals("account_info")) {
            // creating new entry if first time
            if (!TestudoBankRepository.getCustomerStockShares(jdbcTemplate, userID, ticker).isPresent()) {
                TestudoBankRepository.initCustomerStockBalance(jdbcTemplate, userID, ticker);
            }
    ...
    /**
     * Method which is used to sell any stocks user holds
     * 
     * @param user the user
     * @return the string of the page we should render based on conditions
    */
    ...
    public String sellStock(@ModelAttribute("user") User user) {
        ...
        String ticker = user.getWhichStockToSell();
        double shares = user.getAmountToSellStock();
        double price = stockPriceClient.getCurrentStockValue(ticker);
        double costOfStockAfterInvestment = shares * price;
        ...
        // should not happen but on the off chance it does
        if (costOfStockAfterInvestment < 0) {
            return "welcome";
        }

        Optional<Integer> shareAmount = TestudoBankRepository.getCustomerStockShares(jdbcTemplate, userID, ticker);
        if (!shareAmount.isPresent()) {
            return "welcome";
        }

        user.setAmountToDeposit(costOfStockAfterInvestment);
