...
public class MvcController {
    ...
    private int applyInterestRateToPennyAmount(int pennyAmount) {
        if (pennyAmount > 0) {
            ...
            return 0; // the description doesn't mention how to design for the bad case
    }
    ...
    public String submitWithdraw(@ModelAttribute("user") User user) {
        ...
        if (userWithdrawAmtInPennies > userBalanceInPennies) { // if withdraw amount exceeds main balance, withdraw into
            ...
            // int newOverdraftIncreaseAmtAfterInterestInPennies =
            // (int)(excessWithdrawAmtInPennies * INTEREST_RATE);
            int newOverdraftIncreaseAmtAfterInterestInPennies = applyInterestRateToPennyAmount(
                    excessWithdrawAmtInPennies);
