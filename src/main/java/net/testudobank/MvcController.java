...
public class MvcController {
  ...
  public String submitDeposit(@ModelAttribute("user") User user) {
    ...
    if (userOverdraftBalanceInPennies > 0) { // deposit will pay off overdraft first
      ...
      // will check for deposit limit and ii above limit

      // checks for minimum deposit amount needed for intrest to be applied
      if (userDepositAmtInPennies >= 2000) {
        ...
        // increments total number of user deposits

        if (TestudoBankRepository.getCustomerNumberOfDepositsForInterest(jdbcTemplate, userID) == 5) {
          // checks desposit limit

          int userBalanceInPennies = TestudoBankRepository.getCustomerCashBalanceInPennies(jdbcTemplate, userID);
          userBalanceInPennies = (int) (userBalanceInPennies * BALANCE_INTEREST_RATE);
          // Calculates new balance with intrest

          TestudoBankRepository.setCustomerCashBalance(jdbcTemplate, userID, userBalanceInPennies);
          // updates user balance

          TestudoBankRepository.setCustomerNumberOfDepositsForInterest(jdbcTemplate, userID, 0);
          // restets user deposit count to 0
        }
      }
    }
