...
/**
  ...
  public String applyInterest(@ModelAttribute("user") User user) {
    ...
    /*
     *  to count deposit towards number of deposits for interest, minimum $20 deposit and no overdraft
     */
    if (depositAmount >= 20.00
        && TestudoBankRepository.getCustomerOverdraftBalanceInPennies(jdbcTemplate, userID) == 0) {
      ...
      // apply interest after every 5 valid deposits > $20
      if ((numDepositsForInterest + 1) % 5 == 0) {
        ...
        double balanceWithInterest = currentBalanceInPennies * BALANCE_INTEREST_RATE;

        // interest earned
        double interestDeposit = balanceWithInterest - currentBalanceInPennies;

        TestudoBankRepository.increaseCustomerCashBalance(jdbcTemplate, userID, (int) interestDeposit);

        // reset count of deposits for interest to 0
        TestudoBankRepository.setCustomerNumberOfDepositsForInterest(jdbcTemplate, userID, 0);

        // display that the interest has been logged in the TransactionHistory table
        String currentTime = SQL_DATETIME_FORMATTER.format(new java.util.Date());
        TestudoBankRepository.insertRowToTransactionHistoryTable(jdbcTemplate, userID, currentTime,
            TRANSACTION_HISTORY_DEPOSIT_ACTION, (int) interestDeposit);
