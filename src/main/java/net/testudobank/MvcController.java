...
public class MvcController {
  ...
  /**
   * HTML GET request handler that serves the "transfer_crypto_form" page to the
   * user.
   * An empty `User` object is also added to the Model as an Attribute to store
   * the user's transfer form input.
   * 
   * @param model
   * @return "transfer_crypto_form" page
   */
  @GetMapping("/transfercrypto")
  public String showTransferCryptoForm(Model model) {
    User user = new User();
    user.setEthPrice(cryptoPriceClient.getCurrentEthValue());
    user.setSolPrice(cryptoPriceClient.getCurrentSolValue());
    model.addAttribute("user", user);
    return "transfer_crypto_form";
  }
  ...
  /**
   * HTML POST request handler for the Transfer Crypto Form page.
   * <p>
   * The same username+password handling from the login page is used.
   * <p>
   * If the password attempt is correct, and the amount to transfer is a valid
   * amount
   * that does not exceed crypto balance, the crypto amount to transfer will be
   * deducted
   * from the users crypto balance and added to the recipients crypto balance
   * <p>
   * If the password attempt is incorrect or the amount to purchase is invalid,
   * the user is redirected to the "welcome" page.
   * <p>
   ...
   *         "welcome" page.
   */
  @PostMapping("/transfercrypto")
  public String transferCrypto(@ModelAttribute("user") User sender) {
    //// Invalid Input/State Handling ////
    String senderUserID = sender.getUsername();
    String senderPasswordAttempt = sender.getPassword();
    String senderPassword = TestudoBankRepository.getCustomerPassword(jdbcTemplate, senderUserID);
    ...
    // case where customer tries to send crypto to themselves
    if (sender.getTransferRecipientID().equals(senderUserID)) {
      return "welcome";
    }

    // must buy a supported cryptocurrency
    String cryptoToTransfer = sender.getWhichCryptoToTransfer();
    if (MvcController.SUPPORTED_CRYPTOCURRENCIES.contains(cryptoToTransfer) == false) {
      return "welcome";
    }

    // initialize variables for transfer amount
    double transferAmount = sender.getAmountToTransferCrypto();
    ...
    // possible for user to not have any crypto
    Optional<Double> cryptoBalance = TestudoBankRepository.getCustomerCryptoBalance(jdbcTemplate, senderUserID,
        cryptoToTransfer);
    if (!cryptoBalance.isPresent()) {
      return "welcome";
    }
    ...
    // check if user has required crypto balance
    if (cryptoBalance.get() < transferAmount) {
      return "welcome";
    }
    ...
    TestudoBankRepository.decreaseCustomerCryptoBalance(jdbcTemplate, senderUserID, cryptoToTransfer, transferAmount);

    recipient.setCryptoTransaction(true);
    TestudoBankRepository.increaseCustomerCryptoBalance(jdbcTemplate, recipientUserID, cryptoToTransfer,
        transferAmount);

    String currentTime = SQL_DATETIME_FORMATTER.format(new java.util.Date());
    TestudoBankRepository.insertRowToCryptoLogsTable(jdbcTemplate, recipientUserID, cryptoToTransfer,
        "RECIEVED_TRANSFER", currentTime, transferAmount);
    TestudoBankRepository.insertRowToCryptoLogsTable(jdbcTemplate, senderUserID, cryptoToTransfer, "TRANSFER",
