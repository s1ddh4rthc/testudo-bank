...
public class MvcControllerIntegTestHelpers {
  ...
  public static void updateSavingsGoalInDB(DatabaseDelegate dbDelegate, String goalID, double newTargetAmount, double currentAmount, LocalDateTime newDeadline) throws ScriptException {
    // Update the existing savings goal in the SavingsGoals table
    String updateSavingsGoalSql = String.format(
        "UPDATE SavingsGoals SET TargetAmount = %f, CurrentAmount = %f, Deadline = '%s' WHERE GoalID = '%s'",
        newTargetAmount, currentAmount, newDeadline, goalID
    );
    ScriptUtils.executeDatabaseScript(dbDelegate, null, updateSavingsGoalSql);
  }
